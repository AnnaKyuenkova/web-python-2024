...
def create():
    ...
    if request.method == 'POST':
        ...
        try:
            query = 'INSERT INTO users (login, last_name, first_name, middle_name, password_hash, role_id) VALUES (%s, %s, %s, %s, SHA2(%s, 256), %s)'
            cursor = db.connection().cursor(named_tuple=True)
...
def edit(user_id):
    ...
    query = 'SELECT users.*, roles.name as role_name FROM users LEFT JOIN roles on roles.id = users.role_id WHERE users.id=%s'
    with db.connection().cursor(named_tuple=True) as cursor:
        cursor.execute(query, (user_id,))
        user = cursor.fetchone()
